...
function displayProgress() {
  ...
  document.getElementById('quest-completed').textContent =
    userProgress.quests?.length || 0;
  document.getElementById('check-ins').textContent =
...
function displayQuests() {
  ...
  questsList.innerHTML = availableQuests
    .map((quest) => {
...
function startQuest(questId) {
  ...
  alert(
    `${tQuest('quest_starting')}: ${tQuest(quest.titleKey)}\n\n${tQuest('quest_start_hint')}`
  );
...
function completeObjective(questId, objectiveId) {
  ...
  if (!questProgress.completedObjectives.includes(objectiveId)) {
    ...
    if (
      quest &&
      questProgress.completedObjectives.length === quest.objectives.length
    ) {
      ...
      userProgress.quests.push({
        id: questId,
        completed: true,
        reward: tQuest(quest.reward.nameKey),
      });
      alert(
        `${tQuest('quest_completed_msg')}: ${tQuest(quest.reward.nameKey)}`
      );
    }
